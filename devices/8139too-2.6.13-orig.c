...
#  define DPRINTK(fmt, args...) dmm_prtk(KERN_DEBUG "%s: " fmt, __FUNCTION__ , ## args)
#else
					 ...
					 struct net_device **dev_out)
{
	...
	*dev_out = NULL;
	...
	if (dev == NULL) {
		dmm_prtk (KERN_ERR PFX "%s: Unable to alloc new net device\n", pci_name(pdev));
		return -ENOMEM;
	}
	...
	if (!(pio_flags & IORESOURCE_IO)) {
		dmm_prtk (KERN_ERR PFX "%s: region #0 not a PIO resource, aborting\n", pci_name(pdev));
		rc = -ENODEV;
		goto err_out;
	}
	/* check for weird/broken PCI region reporting */
	if (pio_len < RTL_MIN_IO_SIZE) {
		dmm_prtk (KERN_ERR PFX "%s: Invalid PCI I/O region size(s), aborting\n", pci_name(pdev));
		rc = -ENODEV;
		goto err_out;
	}
#else
	...
	if (!(mmio_flags & IORESOURCE_MEM)) {
		dmm_prtk (KERN_ERR PFX "%s: region #1 not an MMIO resource, aborting\n", pci_name(pdev));
		rc = -ENODEV;
		goto err_out;
	}
	if (mmio_len < RTL_MIN_IO_SIZE) {
		dmm_prtk (KERN_ERR PFX "%s: Invalid PCI mem region size(s), aborting\n", pci_name(pdev));
		rc = -ENODEV;
		goto err_out;
	}
#endif
	...
	if (!ioaddr) {
		dmm_prtk (KERN_ERR PFX "%s: cannot map PIO, aborting\n", pci_name(pdev));
		rc = -EIO;
		goto err_out;
	}
	...
	if (ioaddr == NULL) {
		dmm_prtk (KERN_ERR PFX "%s: cannot remap MMIO, aborting\n", pci_name(pdev));
		rc = -EIO;
		goto err_out;
	}
	...
	if (RTL_R32 (TxConfig) == 0xFFFFFFFF) {
		dmm_prtk (KERN_ERR PFX "%s: Chip not responding, ignoring board\n",
			pci_name(pdev));
		rc = -EIO;
		goto err_out;
	}
	...
	dmm_prtk (KERN_DEBUG PFX "%s: unknown chip version, assuming RTL-8139\n",
		pci_name(pdev));
	dmm_prtk (KERN_DEBUG PFX "%s: TxConfig = 0x%lx\n", pci_name(pdev), RTL_R32 (TxConfig));
	tp->chipset = 0;
				       ...
				       const struct pci_device_id *ent)
{
	...
	if (pdev->vendor == PCI_VENDOR_ID_REALTEK &&
		...
		dmm_prtk(KERN_INFO PFX "pci dev %s (id %04x:%04x rev %02x) is an enhanced 8139C+ chip\n",
		       pci_name(pdev), pdev->vendor, pdev->device, pci_rev);
		dmm_prtk(KERN_INFO PFX "Use the \"8139cp\" driver for improved performance and stability.\n");
	}
...
static int rtl8139_open (struct net_device *dev)
{
	...
	if (netif_msg_ifup(tp))
		dmm_prtk(KERN_DEBUG "%s: rtl8139_open() ioaddr %#lx IRQ %d"
...
static void rtl8139_start_thread(struct net_device *dev)
{
	...
	if (tp->thr_pid < 0) {
		dmm_prtk (KERN_WARNING "%s: unable to start kernel thread\n",
			dev->name);
	}
}
...
static int rtl8139_close (struct net_device *dev)
{
	...
	if (tp->thr_pid >= 0) {
		...
		if (ret) {
			dmm_prtk (KERN_ERR "%s: unable to signal thread\n", dev->name);
			return ret;
		}
